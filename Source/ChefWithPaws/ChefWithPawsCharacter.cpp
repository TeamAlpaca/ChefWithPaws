	...
	: Super(ObjectInitializer)
{
	...
	TriggerComponent = ObjectInitializer.CreateDefaultSubobject<UTriggerComponent>(this, TEXT("Trigger"));
	//重叠盒尺寸
	TriggerComponent->InitBoxExtent(FVector(50.0f, 30.0f, 100.0f));
	TriggerComponent->SetRelativeLocation(FVector(30.0f, 0.0f, 0.0f));
	TriggerComponent->SetupAttachment(RootComponent);

	TriggerComponent->OnComponentBeginOverlap.AddDynamic(this, &AChefWithPawsCharacter::OnOverlapBegin);       // 当此组件与某对象重叠时，设置通知
	TriggerComponent->OnComponentEndOverlap.AddDynamic(this, &AChefWithPawsCharacter::OnOverlapEnd);
}
...
void AChefWithPawsCharacter::OnOverlapBegin(class UPrimitiveComponent* OverlappedComp, AActor * OtherActor, UPrimitiveComponent * OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult & SweepResult)
{
	if (OtherActor && (OtherActor != this) && OtherComp)
	{
		...
		TriggerComponent->OnOverlapBegin(this, OtherActor, Direction);
	}
}
