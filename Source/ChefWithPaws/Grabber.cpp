...
void UGrabber::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	FVector Location= OtherActor->GetActorLocation();
		...
		GEngine->AddOnScreenDebugMessage(0, 5.0f, FColor::Yellow, OtherActor->GetActorLocation().ToString());
		GEngine->AddOnScreenDebugMessage(-2, 5.0f, FColor::Yellow, GetOwner()->GetActorRotation().ToString());
}
void UGrabber::Grab()
{
	if (Grabbed) { return; }
	...
	if (ComponentToGrab)
	{
		...
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow, TEXT("grab"));
	}
}
